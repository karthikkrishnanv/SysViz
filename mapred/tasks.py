...
#
	...
	def get_sort_finished_string(self):
		if self.sort_finished == 0:
			return (0)
                st=time.localtime(self.sort_finished/1000)
                return (datetime.datetime(st[0],st[1],st[2],st[3],st[4],st[5]).strftime("%m/%d/%Y %H:%M:%S"))
	def get_shuf_finished(self):
		return self.shuf_finished;
	def get_shuf_finished_string(self):
		if self.shuf_finished == 0:
			return (0)
                st=time.localtime(self.shuf_finished/1000)
                return (datetime.datetime(st[0],st[1],st[2],st[3],st[4],st[5]).strftime("%m/%d/%Y %H:%M:%S"))
	def get_taskid(self):
		return self.tid;
#
