...
ApplyTimeWindowFilter <- function(d) {
  if (filter.choice == TRUE) {
    i0 <- which(strptime(d$Time,'%I:%M:%S %p') > strptime(filter.start,'%I:%M:%S %p'))
    i1 <- which(strptime(d$Time,'%I:%M:%S %p') < strptime(filter.end,'%I:%M:%S %p'))
    ...
    i0 <- which(strptime(d$Time,'%I:%M:%S %p') > strptime(start.window,'%I:%M:%S %p'))
    i1 <- which(strptime(d$Time,'%I:%M:%S %p') < strptime(end.window,'%I:%M:%S %p'))
    if (min(i0) < max(i1))
      filtered <- d[min(i0):max(i1),]
    else
      filtered <- d[,]
...
FillGlobalData <- function() {
  for(i in 1:nnodes) {
    ...
    irq.files[[i]]        <<- paste(log.dir,node.name[i],"/IRQ_intr.csv",sep="/")
    ...
    irq.data[[i]]         <<- read.csv(irq.files[[i]],header <<- TRUE)
...
# Bracket data to find most overlapped time window across nodes 
  for(i in 1:nnodes) { 
    ...
    irq.data[[i]]       <<- ApplyTimeWindowFilter(irq.data[[i]])
  }  
...
PlotData <- function(datalist,col.x,col.y,mul,tag,xlabel="Time",ylabel=col.y) {
  ...
  datalist.merged[,col.x] <- as.numeric(strptime(datalist.merged[,col.x],'%I:%M:%S %p'))
  ...
  if (total > nchunk) {
    ...
    labels <- format(as.POSIXlt(breaks,origin="1970-01-01"),'%I:%M:%S %p')
  }else {
...
PlotMisc <- function(){
  ...
  mul=1
  PlotData(intr.data,"Time","intr.s",mul," intr/s (sum)",ylabel="intr/s")
  for(i in 1:length(irq_array)) {
    irq_val <- irq_array[i]
    irq_subset <- list()
    for(n in 1:nnodes) {
      irq_subset[[n]] <- subset(irq.data[[n]],INTR==irq_val)
    }
    mul=1
    irq_tag <- paste(" intr/s IRQ ",irq_val,sep="")
    PlotData(irq_subset,"Time","intr.s",mul,irq_tag,"Time",irq_tag)
  }
}
